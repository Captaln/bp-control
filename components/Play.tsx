...
import { SmashIt } from './SmashIt';
import { ZenFlow } from './ZenFlow';
import { ColorSort } from './ColorSort';
import { MemoryMatch } from './MemoryMatch';
import { TicTacToe } from './TicTacToe';
import { WhackAMood } from './WhackAMood';
import { Grounding } from './Grounding';
import { Grid3X3, Hammer, Waves, ArrowRight, Beaker, BrainCircuit, X, Zap, Anchor } from 'lucide-react';

export const Play: React.FC = () => {
  const [activeGame, setActiveGame] = useState<'menu' | 'popit' | 'smash' | 'zen' | 'sort' | 'match' | 'tictactoe' | 'whack' | 'grounding'>('menu');

  if (activeGame === 'popit') return <BubbleWrap onBack={() => setActiveGame('menu')} />;
  if (activeGame === 'smash') return <SmashIt onBack={() => setActiveGame('menu')} />;
  if (activeGame === 'zen') return <ZenFlow onBack={() => setActiveGame('menu')} />;
  if (activeGame === 'sort') return <ColorSort onBack={() => setActiveGame('menu')} />;
  if (activeGame === 'match') return <MemoryMatch onBack={() => setActiveGame('menu')} />;
  if (activeGame === 'tictactoe') return <TicTacToe onBack={() => setActiveGame('menu')} />;
  if (activeGame === 'whack') return <WhackAMood onBack={() => setActiveGame('menu')} />;
  if (activeGame === 'grounding') return <Grounding onBack={() => setActiveGame('menu')} />;
